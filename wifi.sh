...
if ! [ $(id -u) = 0 ]; then
banner
echo -e "\n\n${White}[${Red}oldAircrack${White}] Please run the tool as root or use sudo.\n"
exit 1
fi
clear
}
...
banner
echo -e "\n\n${White}[${Red}oldAircrack${White}] No WiFi Interface found to continue.\n"
exit 1
fi
...
echo -e "${Red}
THE OLD-HACKER            "
echo -e "${Yellow} \n             A old script for aircrack-ng, wifi hacking.
      The script only works if your wifi adapter has monitor mode."
echo -e "${Green}\n                    Developed by: OLD-HACKER (3xploitGuy)"
...
sleep 0.5
echo -e "[${Yellow}GitHub${White}] You can discuss at https://github.com/Fahadkhan553/GROUP-JON/discussions"
